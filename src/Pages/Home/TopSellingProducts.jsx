  ...
  return (
    <div className={`autoplay shadow-xl border border-[#a0d3e7] rounded-lg py-6 mb-6 theme-${theme}`}>
      ...
      <Slider {...settings} className="sm:slides-1 md:slides-2 lg:slides-3">
        {products.map((product, index) => (
          <div key={index} className='p-4'>
            <div className={`bg-base-200 rounded-lg shadow-lg overflow-hidden flex items-center theme-${theme}`}>
              <img src={product.image} alt={product.title} className="w-[200px] h-40" />
              <div className="p-4">
                <h2 className={`text-lg font-semibold text-${theme === 'dark' ? 'white' : 'black'}`}>{product.title}</h2>
              </div>
            </div>
          </div>
        ))}
